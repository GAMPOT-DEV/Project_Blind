...
namespace Blind
{
    ...
    public class PlayerCharacter : Character,IGameManagerObj
    {
        ...
        [SerializeField] public Blind.ScriptableObjects.PlayerCharacter playerData;
        ...
        public override void Awake()
        {
            ...
            _defaultSpeed = playerData.maxSpeed;
            //_dashSpeed = 10f;
            //_defaultTime = 0.2f;
            _dashCount = 1;
            ...
            _attack.Init(playerData.attack_x,playerData.attack_y,playerData.damage);
            _paring.Init(playerData.paring_x, playerData.paring_y);
        ...
        public void GroundedHorizontalMovement(bool useInput, float speedScale = 0.1f)
        {
            ...
            float desiredSpeed = useInput ? flip * playerData.maxSpeed * speedScale : 0f;
            float acceleration = useInput && isInputCheck ? playerData.groundAcceleration : playerData.groundDeceleration;
        ...
        public IEnumerator Dash()
        {
            ...
            _candash = false;
            float originalGravity = playerData.gravity;
            float desiredSpeed = GetFacing() * playerData.dashSpeed * 0.1f;
            ...
            yield return new WaitForSeconds(playerData.defaultTime);
            playerData.gravity = originalGravity;
        ...
        public void Jump()
        {
            if (InputController.Instance.Jump.Down)
            {
                if(!(InputController.Instance.DownJump.Held)) { // 아래 버튼을 누르지 않았다면
                    _moveVector.y = playerData.jumpSpeed;
                }
        ...
        public void UpdateJump()
        {
            ...
            {
                _moveVector.y -= playerData.jumpAbortSpeedReduction * Time.deltaTime;
            }
        }
        /// <summary>
        /// 중력을 적용합니다.
        ...
        {
            float desiredSpeed = InputController.Instance.Horizontal.Value * playerData.maxSpeed;

            float acceleration;

            if (InputController.Instance.Horizontal.ReceivingInput)
                acceleration = playerData.airborneAccelProportion;
            else
                acceleration = playerData.airborneDecelProportion;

            _moveVector.x = Mathf.MoveTowards(_moveVector.x, desiredSpeed, acceleration * Time.deltaTime);
        }
        public void GroundedVerticalMovement()
        {
            _moveVector.y -= playerData.gravity * Time.deltaTime;

            if (_moveVector.y < - playerData.gravity * Time.deltaTime * GroundedStickingVelocityMultiplier)
            {
                _moveVector.y = - playerData.gravity * Time.deltaTime * GroundedStickingVelocityMultiplier;
            }
        }
        ...
        {
            return Time.time - _lastClickTime > playerData.maxComboDelay;
        }
        ...
        {
            _moveVector.x = playerData.hurtMove * (float)enemyFacing;
        }
